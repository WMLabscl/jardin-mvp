  ...
  return (
    <section id="niveles" className="relative py-28 lg:py-40 bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-500 overflow-hidden">
      ...
      <div className="container-custom relative z-20 px-4 sm:px-6">
        ...
        {/* --- GRID DE TARJETAS CORREGIDO PARA MÓVIL --- */}
        {/* Usamos gap-y-24 en móvil para que los íconos tengan suficiente espacio para "salir" sin chocar */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-y-24 gap-x-12 lg:gap-10 max-w-7xl mx-auto pb-10 mt-16 md:mt-12">
          {niveles.map((nivel, index) => (
            <motion.div
              ...
              /* Aumentado a mt-14 en móvil para compensar mejor el ícono absoluto */
              className={`mt-14 md:mt-0 ${nivel.translate} h-full`}
            >
              {/* TARJETA BLANCA PURA */}
              <div className={`relative bg-white rounded-[40px] p-8 pt-16 md:p-10 md:pt-16 text-center shadow-2xl transition-all duration-300 ${nivel.shadowHover} hover:-translate-y-2 group flex flex-col h-full`}>
                ...
                {/* ÍCONO ROMPIENDO EL BORDE SUPERIOR */}
